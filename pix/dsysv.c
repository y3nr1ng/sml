 ...
 */
static void mxLU(int N, double *A, int *ipiv, double *L, double *LU)
{
    int     i, j, k;
    double  lij;

/*
    if ((L = (double *)malloc(sizeof(double)*N*N)) == NULL) {
	printf("!!! mxLU: not enough memory.\n");
	exit(1);
    }
*/
...
void dsysv(int N, double *A, int Nrhs, double *B, double *X)
{
    ...
    if (ipiv == NULL || LU == NULL || L == NULL) {
	printf("!!! dsysv: not enough memory.\n");
	exit(1);
    }
    if (X != B)
